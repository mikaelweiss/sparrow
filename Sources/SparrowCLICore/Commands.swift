...
public struct New: ParsableCommand {
    ...
    public func run() throws {
        ...
        // CLAUDE.md and AGENTS.md (identical content)
        let aiInstructions = """
        # \(name)
        ...
        """
        try aiInstructions.write(toFile: projectDir + "/CLAUDE.md", atomically: true, encoding: .utf8)
        try aiInstructions.write(toFile: projectDir + "/AGENTS.md", atomically: true, encoding: .utf8)

        let createdFiles = [
            "\(name)/Package.swift",
            "\(name)/Sources/App.swift",
            "\(name)/.gitignore",
            "\(name)/CLAUDE.md",
            "\(name)/AGENTS.md",
        ]
